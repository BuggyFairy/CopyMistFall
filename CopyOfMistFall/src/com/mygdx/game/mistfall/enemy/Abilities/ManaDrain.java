...
public class ManaDrain {
	...
	public static void addModification(GameController gc,Enemy enemy, EnemyArea dest,Hero heroDest){
		...
		if (dest==EnemyArea.HERO){
			...
			gc.getHeroes().get(heroDest.getHeroID()).getHeroEnemies().getCards().get(enemyPos).updateModification(ModSource.ENEMY, ModType.MANA_DRAIN, ModTarget.GENERAL, 0, enemy.getEnemyID(), enemy.getName());
		}
	}
