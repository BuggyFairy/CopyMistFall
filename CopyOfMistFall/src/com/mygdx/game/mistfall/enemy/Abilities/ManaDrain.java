...
public class ManaDrain {
	...
	public static void updateMoved(GameController gc,Enemy enemy, EnemyArea source, EnemyArea dest){
		...
		// IF an Enemy with the Mana Drain Ability enters a Hero Area, add the Cursed Bolt Modification
		if(dest==EnemyArea.HERO){
			enemy.updateModification(ModSource.ENEMY, ModType.MANA_DRAIN, ModTarget.GENERAL, 0, enemy.getEnemyID());
		}
		// If a Enemy with the Mana Drain Ability moves from a Hero Area in the Quest Area, remove the Cursed Bolt Modification if possible
		if (dest==EnemyArea.QUEST && source==EnemyArea.HERO){
			enemy.removeModification(ModType.MANA_DRAIN, ModSource.ENEMY, enemy.getEnemyID());
		}
	}
