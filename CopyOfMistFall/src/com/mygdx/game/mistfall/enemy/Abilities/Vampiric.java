...
public class Vampiric {
	...
	public static void addModification(GameController gc,Enemy enemy, EnemyArea dest,Hero heroDest){
		...
		if (dest==EnemyArea.HERO){
			...
			gc.getHeroes().get(heroDest.getHeroID()).getHeroEnemies().getCards().get(enemyPos).updateModification(ModSource.ENEMY, ModType.VAMPIRIC, ModTarget.GENERAL, 0, enemy.getEnemyID());
		}
	}
